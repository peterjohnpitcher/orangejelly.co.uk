...
function analyzeAllFields(posts: any[]): FieldAnalysis[] {
  ...
  return fieldMappings.map((field) => analyzeField(posts, field.path, field.name));
}
...
function analyzeIndividualPosts(posts: any[]): PostAnalysis[] {
  ...
  return posts.map((post) => {
    ...
    // Check Local SEO
    if (isFieldEmpty(post.localSEO?.targetLocation))
      incompleteFields.push('localSEO.targetLocation');
    if (isFieldEmpty(post.localSEO?.nearbyLandmarks))
      incompleteFields.push('localSEO.nearbyLandmarks');

    // Check CTA settings
    if (isFieldEmpty(post.ctaSettings?.primaryCTA)) incompleteFields.push('ctaSettings.primaryCTA');
    if (isFieldEmpty(post.ctaSettings?.whatsappMessage))
      incompleteFields.push('ctaSettings.whatsappMessage');
...
function generateCriticalIssues(
  ...
  criticalFields.forEach((field) => {
    ...
    if (analysis && analysis.percentagePresent < 50) {
      issues.push(
        `${field} is missing in ${analysis.missing} posts (${analysis.percentagePresent.toFixed(1)}% complete)`
      );
    }
  });
  ...
  const publishedPostsWithIssues = postAnalysis.filter(
    (p) =>
      p.status === 'published' &&
      (p.missingFields.includes('quickAnswer (CRITICAL)') ||
        p.missingFields.includes('faqs (CRITICAL for Voice Search)'))
  );

  if (publishedPostsWithIssues.length > 0) {
    issues.push(
      `${publishedPostsWithIssues.length} published posts are missing critical SEO fields (quickAnswer or FAQs)`
    );
  }
...
function generateRecommendations(
  ...
  const quickAnswerAnalysis = fieldAnalysis.find(
    (f) => f.fieldName === 'Quick Answer (Critical for Featured Snippets)'
  );
  if (quickAnswerAnalysis && quickAnswerAnalysis.percentagePresent < 80) {
    recommendations.push(
      `• Add Quick Answer to ${quickAnswerAnalysis.missing} posts - this is CRITICAL for featured snippets`
    );
  }
  ...
  if (faqAnalysis && faqAnalysis.percentagePresent < 80) {
    recommendations.push(
      `• Add FAQs to ${faqAnalysis.missing} posts - essential for voice search optimization`
    );
  }
  ...
  if (quickStatsAnalysis && quickStatsAnalysis.percentagePresent < 70) {
    recommendations.push(
      `• Add quick stats to ${quickStatsAnalysis.missing} posts for AI Overview optimization`
    );
  }
...
function formatReport(analysis: SchemaAnalysis): string {
  ...
  analysis.fieldAnalysis
    ...
    .forEach((field) => {
      ...
      report.push(
        `| ${field.fieldName} | ${field.present} | ${field.missing} | ${field.percentagePresent.toFixed(1)}% | ${sampleValues} |`
      );
    });
  ...
  analysis.postAnalysis
    ...
    .forEach((post) => {
      report.push(
        `| ${post.title.substring(0, 30)}... | ${post.status} | ${post.missingFields.length} | ${post.incompleteFields.length} | ${post.totalMissingCount} |`
      );
    });
...
async function main() {
  try {
    ...
    topIncompleteFields.forEach((field, index) => {
      console.log(
        `${index + 1}. ${field.fieldName}: ${field.percentagePresent.toFixed(1)}% complete (${field.missing} missing)`
      );
    });
