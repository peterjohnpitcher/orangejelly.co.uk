...
import { writeClient } from '../src/lib/sanity.write-client.js';
...
function processText(text: string): { newText: string; changes: Array<{ oldText: string; newText: string; reason: string }> } {
  let newText = text;
  const changes: Array<{ oldText: string; newText: string; reason: string }> = [];
...
async function fixAllBlogPosts() {
  ...
  try {
    ...
    const changesByReason: { [key: string]: number } = {};
...
export { fixAllBlogPosts, processBlogPost };