...
export function generateRSSFeed(): string {
  ...
  const lastBuildDate = posts.length > 0 && posts[0].publishedDate 
...
export function generateJSONFeed(): string {
  ...
  const jsonFeed = {
    ...
    items: posts.map(post => ({
      ...
      date_modified: (post.updatedDate ? new Date(post.updatedDate) : new Date(post.publishedDate || Date.now())).toISOString(),
      author: {
        name: siteConfig.author.name,
        email: siteConfig.author.email
      },
...
export function saveFeeds(): void {
  ...
  try {
    ...
    console.log(`- Sitemap: ${siteConfig.url}/sitemap.xml`);
    console.log(`- Robots: ${siteConfig.url}/robots.txt`);
