...
// Cache variables
let cachedServicesPageContent: ServicesPageContent | null = null;
let cachedServicePackages: ServicePackage[] | null = null;
let cachedServicesFAQs: ServicesFAQ[] | null = null;

// Hardcoded fallback data from the current ServicesPage.tsx
const FALLBACK_SERVICES_PAGE: ServicesPageContent = {
  _id: 'servicesPage',
  title: 'Services Page',
  hero: {
...
// Hardcoded fallback service packages
const FALLBACK_SERVICE_PACKAGES: ServicePackage[] = [
  {
    _id: 'recovery',
...
const FALLBACK_SERVICES_FAQS: ServicesFAQ[] = [
  {
    _id: '1',
...
/**
 * Fetch services page content from Sanity
 */
export async function getServicesPageContent(): Promise<ServicesPageContent> {
  if (cachedServicesPageContent) {
    return cachedServicesPageContent;
  }

  try {
    const content = await client.fetch<ServicesPageContent>(servicesPageQuery);
    if (content && content.title) {
      cachedServicesPageContent = content;
      return content;
    }
  } catch (error) {
    console.error('Error fetching services page content from Sanity:', error);
  }
...
/**
 * Fetch service packages from Sanity
 */
export async function getServicePackages(): Promise<ServicePackage[]> {
  if (cachedServicePackages) {
    return cachedServicePackages;
  }

  try {
    const packages = await client.fetch<ServicePackage[]>(servicePackagesQuery);
    if (packages && packages.length > 0) {
      cachedServicePackages = packages;
      return packages;
    }
  } catch (error) {
    console.error('Error fetching service packages from Sanity:', error);
  }

  // Return fallback data
  return FALLBACK_SERVICE_PACKAGES;
}

/**
 * Fetch services FAQs from Sanity
 */
export async function getServicesFAQs(): Promise<ServicesFAQ[]> {
  if (cachedServicesFAQs) {
    return cachedServicesFAQs;
  }

  try {
    const faqs = await client.fetch<ServicesFAQ[]>(servicesFAQsQuery);
    if (faqs && faqs.length > 0) {
      cachedServicesFAQs = faqs;
      return faqs;
    }
  } catch (error) {
    console.error('Error fetching services FAQs from Sanity:', error);
  }

  // Return fallback data
  return FALLBACK_SERVICES_FAQS;
}
...
export function clearServicesCache(): void {
  cachedServicesPageContent = null;
  cachedServicePackages = null;
  cachedServicesFAQs = null;
}