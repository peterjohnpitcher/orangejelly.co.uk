...
import { ArrowRight, Calendar, CheckCircle2, MapPin, Users, Zap, Trophy, Target, Heart } from 'lucide-react';
      ...
      <main className="min-h-screen bg-gradient-to-b from-cream to-white">
        ...
        <Section background="cream" padding="large" className="relative">
          ...
          <div className="text-center space-y-4">
            <Badge variant="outline" className="mb-4">Est. 2019</Badge>
            ...
            <Text className="text-xl text-charcoal/80 max-w-3xl mx-auto">
              We run The Anchor in Stanwell Moor. We discovered how AI saves 25 hours a week. 
              Now we help other licensees do the same.
            </Text>
        ...
        <div className="container mx-auto max-w-4xl">
          <Heading level={2} className="text-3xl font-bold text-center mb-8">Our Journey</Heading>
          ...
          <Tabs defaultValue="story" className="w-full">
            ...
            <TabsContent value="timeline" className="mt-6">
              <div className="space-y-4">
                {[
                  { date: 'March 2019', icon: <Calendar className="h-5 w-5" />, title: 'Took Over The Anchor', desc: 'Empty tables, no strategy, pure hope.' },
                  { date: '2021', icon: <Zap className="h-5 w-5" />, title: 'Discovered AI', desc: 'Started testing, failing, learning.' },
                  { date: 'Jan-Feb 2024', icon: <Trophy className="h-5 w-5" />, title: 'Transformation', desc: 'Six brutal weeks forced full AI adoption. GP hit 71%.' },
                  { date: 'Today', icon: <Target className="h-5 w-5" />, title: 'Helping Others', desc: 'Helping licensees save 5+ hours weekly with proven tools.' },
                ].map((item, i) => (
                  <Card key={i}>
                    <CardHeader>
            ...
            <TabsContent value="values" className="mt-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  { icon: '🎯', title: 'Real Experience', desc: 'We run an actual pub. Every strategy tested first.' },
                  { icon: '💰', title: 'Honest Pricing', desc: '£62.50 per hour plus VAT. No packages, no surprises.' },
                  { icon: '🛡️', title: 'Guaranteed Results', desc: '30-day money-back guarantee. We believe in what we do.' },
                  { icon: '🤝', title: 'Personal Service', desc: 'Just me, no sales team. Direct access to someone who understands.' },
                ].map((value, i) => (
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-4">
                <Avatar className="h-20 w-20">
                  <AvatarImage 
                    src={aboutContent?.founderSection?.image?.asset 
                      ? urlFor(aboutContent.founderSection.image).width(160).height(160).url() 
                      : "/images/peter-and-billy-anchor.jpg"} 
                    alt="Peter Pitcher, founder of Orange Jelly" 
                  />
                  <AvatarFallback>PP</AvatarFallback>
                </Avatar>
                <div>
                  <Heading level={2} className="text-3xl font-bold">{aboutContent?.founderSection?.name || "Meet Peter Pitcher"}</Heading>
                  <Text className="text-muted-foreground">{aboutContent?.founderSection?.role || "Founder & Pub Owner"}</Text>
                </div>
              </div>
              ...
              <Card className="bg-teal text-white">
                <CardContent className="pt-6">
                  <Text className="italic text-lg">
                    "If I can run a successful pub while working full-time, imagine what you can achieve 
                    with the right AI tools supporting you."
                  </Text>
                </CardContent>
              </Card>
            </div>
        ...
        <div className="container mx-auto max-w-4xl">
          <Card className="bg-gradient-to-br from-orange-light to-orange">
            <CardHeader>
              <CardTitle className="text-2xl text-white">Quick Facts About Orange Jelly</CardTitle>
            </CardHeader>
            <CardContent>
        ...
        {/* FAQs Section */}
        <Section background="cream" padding="large" className="aria-label-frequently-asked-questions">
        <div className="container mx-auto max-w-3xl">
          <Heading level={2} className="text-3xl font-bold text-center mb-8">Your Questions Answered</Heading>
        ...
        <div className="container mx-auto max-w-6xl">
          <Heading level={2} className="text-3xl font-bold text-center mb-8">Our Partners & Memberships</Heading>
          <div className="flex flex-wrap justify-center items-center gap-8">
            <Badge variant="secondary" className="text-lg px-6 py-2">Greene King Tenant</Badge>
            <Badge variant="secondary" className="text-lg px-6 py-2">BII Member</Badge>
            <Badge variant="secondary" className="text-lg px-6 py-2">Featured in BII Magazine Autumn 2025</Badge>
          </div>
        </div>
      </Section>
        ...
        <div className="container mx-auto max-w-4xl text-center">
          <Heading level={2} className="text-3xl font-bold mb-6">Come See The Results Yourself</Heading>
          ...
          <Card className="inline-block">
            ...
            <CardFooter>
              <Button variant="link" asChild className="w-full">
                <Link href="https://maps.google.com/?q=The+Anchor+Stanwell+Moor" target="_blank" rel="noopener noreferrer">
        ...
        <div className="container mx-auto max-w-4xl text-center">
          <Heading level={2} className="text-4xl font-bold mb-6">Ready to Transform Your Pub?</Heading>
          <Text className="text-xl mb-8">
          ...
          <div className="flex gap-4 justify-center">
            ...
            <Button size="lg" variant="outline" className="bg-transparent text-white border-white hover:bg-white hover:text-orange" asChild>
