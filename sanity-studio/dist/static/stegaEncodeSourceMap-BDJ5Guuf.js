import { b7 as I, b8 as G } from './sanity-BOkIjAR6.js';
const D = /_key\s*==\s*['"](.*)['"]/;
function H(t) {
...
function K(t) {
  return `$${t.map((e) => (typeof e == 'string' ? `['${e.replace(/[\f\n\r\t'\\]/g, (r) => x[r])}']` : typeof e == 'number' ? `[${e}]` : e._key !== '' ? `[?(@._key=='${e._key.replace(/['\\]/g, (r) => x[r])}')]` : `[${e._index}]`)).join('')}`;
}
function j(t) {
  const e = [],
    r = /\['(.*?)'\]|\[(\d+)\]|\[\?\(@\._key=='(.*?)'\)\]/g;
...
function Y(t, e, r) {
  return $(t, (i, n) => {
    ...
    const f = e.documents[o.source.document],
      u = e.paths[o.source.path],
      h = j(c),
      l = j(u).concat(n.slice(h.length));
    return r({ sourcePath: l, sourceDocument: f, resultPath: n, value: i });
  });
}
const Z = 'drafts',
  v = 'versions',
...
function et(t) {
  if (!S(t)) return;
  const [e, r, ...i] = t.split(m);
  return r;
}
function nt(t) {
  return S(t) ? t.split(m).slice(2).join(m) : N(t) ? t.slice(C.length) : t;
}
function rt(t) {
  const {
    baseUrl: e,
  ...
  const u = r === 'default' ? void 0 : r,
    ...
    l = nt(n),
    k = Array.isArray(o) ? L(W(o)) : o,
    d = new URLSearchParams({ baseUrl: e, id: l, type: s, path: k });
  if (
    (u && d.set('workspace', u),
    h && d.set('tool', h),
    c && d.set('projectId', c),
  ...
  const y = [e === '/' ? '' : e];
  u && y.push(u);
  const w = ['mode=presentation', `id=${l}`, `type=${s}`, `path=${encodeURIComponent(k)}`];
  return (h && w.push(`tool=${h}`), y.push('intent', 'edit', `${w.join(';')}?${d}`), y.join('/'));
}
function it(t) {
...
function pt(t) {
  try {
    const { protocol: e } = new URL(t, t.startsWith('/') ? 'https://acme.com' : void 0);
    return at.has(e) || e.startsWith('web+');
  } catch {
    return !1;
  }
}
...
const P = 20;
function ct(t, e, r) {
  var f, u, h, l, k, d, y, w, E;
  const { filter: i, logger: n, enabled: s } = r;
  if (!s) {
    const a = "config.enabled must be true, don't call this function otherwise";
    throw (
      (f = n == null ? void 0 : n.error) == null ||
        f.call(n, `[@sanity/client]: ${a}`, { result: t, resultSourceMap: e, config: r }),
      new TypeError(a)
    );
  }
  if (!e)
  ...
  if (!r.studioUrl) {
    ...
    throw (
      (h = n == null ? void 0 : n.error) == null ||
        h.call(n, `[@sanity/client]: ${a}`, { result: t, resultSourceMap: e, config: r }),
      new TypeError(a)
    );
  }
  const o = { encoded: [], skipped: [] },
    c = Y(t, e, ({ sourcePath: a, sourceDocument: _, resultPath: b, value: p }) => {
      if (
        (typeof i == 'function'
          ? i({ sourcePath: a, resultPath: b, filterDefault: T, sourceDocument: _, value: p })
          : T({ sourcePath: a, resultPath: b, value: p })) === !1
      )
        return (
          n &&
            o.skipped.push({
              path: O(a),
              value: `${p.slice(0, P)}${p.length > P ? '...' : ''}`,
              length: p.length,
            }),
          p
        );
      ...
      if (!R) return p;
      const { _id: q, _type: J, _projectId: F, _dataset: z } = _;
      return G(
        p,
        {
          origin: 'sanity.io',
          href: rt({
            ...
            type: J,
            path: a,
            ...(!r.omitCrossDatasetReferenceData && { dataset: z, projectId: F }),
          }),
        },
        !1
      );
  ...
  if (n) {
    ...
    if (
      ((a || _) &&
        ((l = (n == null ? void 0 : n.groupCollapsed) || n.log) == null ||
      ...
      o.encoded.length > 0 &&
        ((d = n == null ? void 0 : n.log) == null ||
          d.call(n, '[@sanity/client]: Table of encoded paths'),
        (y = (n == null ? void 0 : n.table) || n.log) == null || y(o.encoded)),
      o.skipped.length > 0)
    ) {
      const b = new Set();
      for (const { path: p } of o.skipped) b.add(p.replace(D, '0').replace(/\[\d+\]/g, '[]'));
      (w = n == null ? void 0 : n.log) == null ||
        w.call(n, '[@sanity/client]: List of skipped paths', [...b.values()]);
    }
    (a || _) && ((E = n == null ? void 0 : n.groupEnd) == null || E.call(n));
  }
  return c;
}
function O(t) {
